const Rocky = {
    ...
    fragmentShader: `
        ...
        void main(void) {
            ...
            float noiseBase = (noise(warp, 0.006, 0.8) + 1.0) / 2.0;

            float frequency = 0.06;
            float brightNoise = snoise(vTexCoord3D * 0.015) * 1.4 - 0.9;

            float craterNoise = snoise(vTexCoord3D * 0.07);
            float craterPlus = max(0.0, craterNoise * 3.0 - 2.0);
            float craterMinus = max(0.0, craterNoise * 5.0 - 3.8);
            float smallCraters = max(0.0, craterPlus - craterMinus) * 0.3;
